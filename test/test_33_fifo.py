...
async def test_fifo(dut):
    ...
    items = []
    assert await sample(dut, FIFO, 0) == EMPTY
    ...
    async def command(operation, value=0):
        ...
        if operation == PUSH and len(items) < 4:
            ...
            items.pop(0)
        output = await step(dut, FIFO, value, operation)
    ...
    assert await sample(dut, FIFO, 0) == expected_output(items)
