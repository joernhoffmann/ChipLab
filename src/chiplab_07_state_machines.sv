...
module chiplab_07_state_machines (
    ...
    always_comb begin
        ...
        case (moore_state)
            INACTIVE: if (start && !stop) moore_next = ACTIVE;
            ACTIVE: if (stop) moore_next = INACTIVE;
            default: moore_next = INACTIVE;
        endcase
    end
    ...
    always_comb begin
        ...
        case (mealy_state)
            INACTIVE: if (start && !stop) mealy_next = ACTIVE;
            ACTIVE: if (stop) mealy_next = INACTIVE;
            default: mealy_next = INACTIVE;
        endcase
    end
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)
    ...
    // ------------------------------------------------------------------------
    localparam [1:0] RED = 2'd0, RED_AMBER = 2'd1, GREEN = 2'd2, AMBER = 2'd3;
    ...
    // ------------------------------------------------------------------------
    localparam [1:0] IDLE = 2'd0, BUSY = 2'd1, ACK = 2'd2;
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n)
            ...
            case (handshake_state)
                IDLE: if (request) handshake_state <= BUSY;
                BUSY: if (complete) handshake_state <= ACK;
                ACK: if (!request) handshake_state <= IDLE;
                default: handshake_state <= IDLE;
            endcase
        end
    end
    assign handshake_result = {4'b0, handshake_state,
                               handshake_state == BUSY, handshake_state == ACK};
    ...
    localparam [2:0] PARK_IDLE = 3'd0, ENTER_A = 3'd1, ENTER_AB = 3'd2,
                     ENTER_B = 3'd3, EXIT_B = 3'd4, EXIT_AB = 3'd5,
                     EXIT_A = 3'd6, WAIT_CLEAR = 3'd7;
    ...
    always_comb begin
        ...
        case (parking_state)
            ...
            EXIT_A: case (sensors)
                ...
                default: ;
            endcase
            default: if (sensors == 2'b00) parking_next = PARK_IDLE;
        endcase
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            // Event pulses last one clock, including when the experiment is paused.
            entered <= 1'b0;
            exited <= 1'b0;
    ...
    assign parking_result = {parking_state == WAIT_CLEAR, parking_state != PARK_IDLE,
                             exited, entered, occupancy};
